...
var defaultRenderer = (field) => {
  return <atom-text-editor ref={field.id} id={field.id} attributes={{mini: true}}></atom-text-editor>
}
...
renderer.select = (field, items) => {
  var i = items ? items : []
  return <select ref={field.id} id={field.id}>
    {i.map( (item) => <option value={item.value}>{item.display}</option>)}
  </select>
}
